multibranchPipelineJob('recommendations-etl-warranties-consumer') {
    branchSources {
        configure { node ->
          node / sources(class: 'jenkins.branch.MultiBranchProject$BranchSourceList') / data / 'jenkins.branch.BranchSource' / source(class: 'jenkins.plugins.git.GitSCMSource') {
            remote 'ssh://git@gitlab.deveng.systems:2222/re-recommendations/recommendations-etl-warranties-consumer.git'
