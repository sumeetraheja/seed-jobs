multibranchPipelineJob('shared-projects-docker-spring-releases') {
    triggers {
        cron('* * * * *')
    }
    ...
    branchSources {
        configure { node ->
            node / sources(class: 'jenkins.branch.MultiBranchProject$BranchSourceList') / data / 'jenkins.branch.BranchSource' / source(class: 'jenkins.plugins.git.GitSCMSource') {
                remote 'ssh://git@gitlab.deveng.systems:2222/shared-projects/shared-docker-spring-releases.git'
